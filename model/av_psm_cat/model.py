...
import time
from causalml.match import smd, create_table_one, NearestNeighborMatch
...
class Model:
    ...
    def fit(self, F, y, datainfo,timeinfo):
        ...
        self.X = self.cat_encs.fit_transform(F['CAT']).values
        self.y = y
    ...
    def predict(self, F,datainfo,timeinfo):
        ...
        X = self.cat_encs.transform(F['CAT']).values
        ...
        psm = NearestNeighborMatch(replace=True, ratio=10, caliper=1, random_state=SEED)

        df_all = pd.DataFrame(X_all, columns=[f'col_{x}' for x in range(X.shape[1])])
        df_all['is_test'] = y_all
        df_all['ps'] = ps_all

        logging.info(f'AV: propensity score matching...')
        df_matched = psm.match(df_all, treatment_col='is_test', score_cols=['ps'])
        df_matched.drop_duplicates(inplace=True)
        logging.info(f'AV: original data:\n{df_all.is_test.value_counts()}')
        logging.info(f'AV: matched data:\n{df_matched.is_test.value_counts()}')

        trn_matched_idx = df_matched.index[df_matched.is_test == 0]

        # Training
        X_trn, X_val, y_trn, y_val = train_test_split(self.X[trn_matched_idx], self.y[trn_matched_idx], test_size=.25, random_state=SEED)
